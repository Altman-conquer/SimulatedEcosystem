    ...
    : QMainWindow(parent)
{
	...
	test_species[Species::Wolf] = 200;
	test_species[Species::Grass] = 200;
	test_species[Species::Cow] = 200;
...
void SimulatedEcosystem::update_Surface() {
    time++;
    ui.time->setText(QString::fromLocal8Bit("时间：") + QString::number(time) + QString::fromLocal8Bit(" 天"));
    environment->Update();
	shared_ptr<vector<shared_ptr<Animal>>> animals = environment->GetEnvironment();
